...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`w-full max-w-[402px] fixed rounded-b-[30px] flex items-center justify-between py-6 px-7 z-50 
        ${isMenuOpen ? '' : 'bg-primary-green'}`}>
        <BurgerButton isOpen={isMenuOpen} toggleMenu={toggleMenu} />
        <Link href="/" className={`text-section-header font-bold text-white ${isMenuOpen ? 'hidden' : ''}`}>
      ...
      >
        ...
        <div className='pt-[62px] w-full flex flex-col items-center gap-8'>
          <Link href="/" className='flex flex-row gap-5 text-section-header text-black justify-center items-center'>
            <BarnIcon />Home
          </Link>
          <Link href="/" className='flex flex-row gap-5 text-section-header text-black justify-center items-center'>
            <ChartIcon />Analyze
